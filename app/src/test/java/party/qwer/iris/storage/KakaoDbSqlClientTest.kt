...
class KakaoDbSqlClientTest {
    private fun resultOf(columns: List<String>, vararg rows: List<String?>): QueryExecutionResult {
    ...
    @Test fun `query maps rows through spec mapper`() {
    ...
    @Test fun `query passes bind args as string array`() {
        var capturedArgs: Array<String?>? = null
        val client = KakaoDbSqlClient { _, args, _ -> capturedArgs = args; QueryExecutionResult(emptyList(), emptyList()) }
    ...
    @Test fun `query passes maxRows to executor`() {
        var capturedMaxRows = 0
        val client = KakaoDbSqlClient { _, _, max -> capturedMaxRows = max; QueryExecutionResult(emptyList(), emptyList()) }
        val spec = QuerySpec(sql = "SELECT 1", bindArgs = emptyList(), maxRows = 500, mapper = { })
        client.query(spec)
        assertEquals(500, capturedMaxRows)
    }

    @Test fun `querySingle returns first result`() {
    ...
    @Test fun `querySingle returns null for empty result`() {
    ...
    @Test fun `empty bind args passes null array`() {
