...
pub struct EventsView { events: Vec<String>, state: ListState, paused: bool, filter_active: bool }

impl EventsView {
    pub fn new() -> Self { Self { events: vec![], state: ListState::default(), paused: false, filter_active: false } }
    pub fn push_event(&mut self, event: SseEvent) {
        ...
        self.state.select(Some(self.visible_events().len().saturating_sub(1)));
    }

    fn visible_events(&self) -> Vec<&str> {
        self.events.iter()
            .filter(|e| !self.filter_active || e.contains("JOIN") || e.contains("LEAVE") || e.contains("KICK") || e.contains("NICK"))
            .map(|e| e.as_str())
            .collect()
    }
}
...
impl View for EventsView {
    fn render(&self, frame: &mut Frame, area: Rect) {
        let title = if self.paused {
            if self.filter_active { " Events (PAUSED, FILTERED) " } else { " Events (PAUSED) " }
        } else if self.filter_active {
            " Events (LIVE, FILTERED) "
        } else {
            " Events (LIVE) "
        };
        let items: Vec<ListItem> = self.visible_events().into_iter().map(ListItem::new).collect();
    ...
    fn handle_key(&mut self, key: KeyEvent) -> ViewAction {
        match key.code {
            ...
            KeyCode::Char('c') => { self.events.clear(); self.state.select(None); ViewAction::None }
            KeyCode::Char('f') => {
            ...
            KeyCode::Down | KeyCode::Char('j') => { let i = self.state.selected().unwrap_or(0);
                self.state.select(Some((i + 1).min(self.visible_events().len().saturating_sub(1)))); ViewAction::None }
