...
pub struct StatsView { pub chat_id: Option<i64>, pub stats: Option<StatsResponse>, pub period: String }

impl StatsView {
    pub fn new() -> Self { Self { chat_id: None, stats: None, period: "7d".to_string() } }
    ...
    fn render(&self, frame: &mut Frame, area: Rect) {
        ...
        if let Some(stats) = &self.stats {
            let chunks = Layout::default().direction(Direction::Vertical)
                .constraints([Constraint::Length(3), Constraint::Min(0)]).split(inner);
            let summary = format!("Total: {} msgs | Active: {} members", stats.total_messages, stats.active_members);
