...
- outbound webhook 전송은 file-backed outbox + route/room partition worker가 담당합니다.
- route별 전송은 room partition 기준으로 backlog를 격리하고 restart 후에도 pending delivery를 재개합니다.
- 보호 API(`/config`, `/reply`, `/query`)는 `X-Iris-Timestamp` / `X-Iris-Nonce` / `X-Iris-Signature` 기반 signed request만 허용합니다.
...
AppRuntime (lifecycle orchestrator)
    ...
    │    └─ OutboxRoutingGateway → FileWebhookOutboxStore (durable outbox)
...
```json
{
  ...
  "webhookToken": "<REQUIRED_SHARED_TOKEN>",
  "botToken": "<LEGACY_UNUSED>",
  "dbPollingRate": 100,
  "messageSendRate": 50
}
```
...
- `webhookToken`: outbound webhook 전송 시 `X-Iris-Token` 헤더로 전달되는 공유 토큰이며, 동시에 보호 API(`/config`, `/reply`, `/query`) 호출 시 signed request HMAC 서명 키로도 사용됩니다.
- `botToken`: config에 존재하지만 현재 인증/서명 로직에서는 사용되지 않는 legacy 필드입니다. 향후 제거 예정입니다.
- `webhookToken`이 비어 있으면 보호 API는 열리지 않으며 `503 service unavailable`으로 거부됩니다.
...
- `GET /config`
- `POST /config/{name}`, `POST /reply`, `POST /reply-image`, `POST /reply-markdown`, `POST /query`
...
- `webhookToken`이 비어 있으면 보호 API는 `503 service unavailable`, 서명이 맞지 않으면 `401 unauthorized`를 반환합니다.
...
SIGNATURE=$(printf '%s' "$CANONICAL" | openssl dgst -sha256 -hmac "$WEBHOOK_TOKEN" -hex | awk '{print $2}')
...
*   **`/reply`**: 카카오톡 채팅방에 메시지 또는 사진을 전송 큐에 등록합니다.
    ...
    **예시 (텍스트 메시지):**
    ...
    `webhookToken`은 보호 API의 HMAC 서명 키로 사용됩니다. 비어 있으면 `503 service unavailable`,
    ...
    image/image_multiple은 `/reply-image`와 같은 native bridge 파이프라인을 사용합니다.
    현재 image reply 실측 기준 `threadScope=2`는 thread-only, `threadScope=3`은 thread + room 같이 보내기입니다.

*   **`/reply-image`**: image/image_multiple 전용 호환 alias 입니다.

    내부적으로는 `/reply`의 이미지 전송과 같은 native bridge 경로를 사용합니다.
    image 전송만 분리해 호출하던 기존 caller를 위해 유지합니다.

*   **`/reply-markdown`**: markdown 메타데이터가 필요한 텍스트를 카카오 share path로 전송 큐에 등록합니다.

    현재는 `/reply`의 in-thread text도 같은 share/graft 경로를 사용합니다.
    `/reply-markdown`은 `attachment.markdown=true`를 강제로 켜는 호환 alias로 보면 됩니다.
    ...
    **예시:**
    ...
    curl -X POST \
      ...
      -H "X-Iris-Nonce: $NONCE" \
      -H "X-Iris-Signature: $SIGNATURE" \
      -d '{"type": "text", "room": "1234567890", "data": "**안녕!**"}' \
      http://[YOUR_DEVICE_IP]:3000/reply-markdown
    ```
...
*   **`/query`**: 카카오톡 데이터베이스에 read-only SQL 쿼리를 실행합니다.

    현재는 `SELECT`, `WITH ... SELECT`, 제한된 read-only `PRAGMA`만 허용합니다.
    multi-statement(`;`)는 거절합니다.
    요청 본문은 `query`, 선택형 `bind` 배열, 선택형 `decrypt` 플래그를 받습니다.
    응답에는 `rowCount`, `columns`, `rows`가 반환됩니다.
    ...
    **예시:**
    ...
    curl -X POST \
      ...
      -H "X-Iris-Nonce: $NONCE" \
      -H "X-Iris-Signature: $SIGNATURE" \
      -d '{"query": "SELECT _id, chat_id, message FROM chat_logs WHERE chat_id = ? ORDER BY _id DESC LIMIT 5", "bind": ["12345"], "decrypt": false}' \
    ...
    `decrypt=true`일 때만 decrypt overlay가 적용됩니다.
...
- 보호 API를 signed request HMAC 계층 하나로 통일
- DB polling과 webhook delivery를 durable checkpoint + file-backed outbox로 분리
...
- `AdminQueryService` 도입 — `/query` 실행 계층을 서버 라우팅에서 분리
...
- 현재 구현은 file-backed outbox와 checkpoint를 사용하며 restart 후에도 in-flight/pending delivery를 재개합니다.
