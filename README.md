...
AppRuntime (lifecycle orchestrator)
    ...
    │    └─ allowlisted query routes (`/query/room-summary`, `/query/member-stats`, `/query/recent-threads`, `/query/recent-messages`)
...
- `POST /config/{name}`, `POST /reply`, `POST /query/room-summary`, `POST /query/member-stats`, `POST /query/recent-threads`, `POST /query/recent-messages`
...
*   **`/query/room-summary`**, **`/query/member-stats`**, **`/query/recent-threads`**, **`/query/recent-messages`**:
