...
| `bridgeToken` | O | app process ↔ LSPosed bridge 간 UDS handshake 토큰 |
...
# → {"status":"ready"} 또는 503 + 미설정 항목 안내
```

`/ready`가 503을 반환하면 응답의 `message` 필드에서 원인을 확인할 수 있습니다.
