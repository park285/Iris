...
`/data/local/tmp/config.json` (또는 `IRIS_CONFIG_PATH`)에서 라우트별 webhook을 설정합니다.
...
설명:
- `webhooks`: 명령 prefix/room 정책에 따라 라우팅할 대상 endpoint 맵
...
| Route | Status | Note |
|---|---|---|
| `hololive` | verified | 현재 확인된 유일한 운영 webhook 경로 |
| `twentyq` | disabled | 현재 사용하지 않아 운영 config에서 비활성화 |
| `turtle-soup` | disabled | 현재 사용하지 않아 운영 config에서 비활성화 |
...
*   **`/config` (GET)**: 현재 구성 조회
*   **`/config/endpoint` (POST)**: 기본 웹훅 엔드포인트(`hololive` 라우트) 업데이트  
    (`twentyq`, `turtle-soup`는 config 파일 또는 환경변수로 별도 설정)
