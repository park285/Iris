...
- `inboundSigningSecret`: 보호 API(`/config`, `/reply`, `/query`) 호출 시 signed request HMAC 서명 키입니다.
- `outboundWebhookToken`: outbound webhook 전송 시 `X-Iris-Token` 헤더로 전달되는 토큰입니다.
- `botControlToken`: bot control 경로에서 사용하는 role-specific secret 필드입니다.
- `inboundSigningSecret`이 비어 있으면 보호 API는 열리지 않으며 `503 service unavailable`으로 거부됩니다.
...
- `inboundSigningSecret`이 비어 있으면 보호 API는 `503 service unavailable`, 서명이 맞지 않으면 `401 unauthorized`를 반환합니다.

**보호 API 호출 예시 (bash):**

```bash
# 보호 API 호출 예시 (bash)
INBOUND_SIGNING_SECRET="<inboundSigningSecret>"
...
SIGNATURE=$(printf '%s' "$CANONICAL" | openssl dgst -sha256 -hmac "$INBOUND_SIGNING_SECRET" -hex | awk '{print $2}')
...
*   **`/reply`**: 카카오톡 채팅방에 메시지 또는 사진을 전송 큐에 등록합니다.
    ...
    **예시 (텍스트 메시지):**
    ...
    `inboundSigningSecret`은 보호 API의 HMAC 서명 키로 사용됩니다. 비어 있으면 `503 service unavailable`,
