...
- outbound webhook 전송은 앱 내부 비동기 큐 + route별 코루틴 워커가 담당합니다.
- route별 전송은 각 봇별 전용 coroutine worker로 서로의 backlog를 격리합니다.
...
카카오톡 메시지
    ↓
Iris (DB polling → route별 비동기 큐)
    → route별 코루틴 워커 → HTTP POST (h2c 우선)
Bot Webhook
    ↓
카카오톡 응답 전송 (Iris /reply)
    ↓ (thread 이미지인 경우)
local UDS → LSPosed bridge → scope/threadId 적용 → 카카오톡 전송
```
