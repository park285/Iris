...
- image reply의 thread 내 전송은 Frida hook 기반 graft 방식으로 지원합니다.
...
카카오톡 응답 전송 (Iris /reply)
    ↓ (thread 이미지인 경우)
Frida hook → scope/threadId 주입 → 카카오톡 전송
```
...
- `GET /config`
- `POST /config/{name}`, `POST /reply`, `POST /reply-markdown`, `POST /query`
...
*   **`/reply`**: 카카오톡 채팅방에 메시지 또는 사진을 전송 큐에 등록합니다.
    ...
    **예시 (텍스트 메시지):**
    ...
    image thread 전송은 Frida hook 기반 graft 방식으로 동작합니다(`frida/` 참조).

*   **`/reply-markdown`**: markdown 메타데이터가 필요한 텍스트를 카카오 share activity 기반으로 전송 큐에 등록합니다.

    일반 `/reply`와 달리 Kakao text share 경로를 사용합니다.
    현재 목적은 `attachment.markdown=true`가 붙는 텍스트 전송입니다.
...
- image reply의 `threadId`/`threadScope` 지원 — Frida hook 기반 thread graft
