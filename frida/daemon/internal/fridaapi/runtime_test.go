...
func TestParseHealthMessageAcceptsDirectObjectPayload(t *testing.T) {
	message := `{"type":"send","payload":{"type":"graft-health","event":"heartbeat","state":"READY","ready":true,"hooks":{"b6":true,"o":true,"t":true,"A":true,"u":true},"counters":{"capture":5},"timestampMs":1711363200000}}`

	health, ok := ParseHealthMessage(message)
	if !ok {
		t.Fatal("ParseHealthMessage returned ok=false for direct object payload")
	}

	if health.Event != "heartbeat" {
		t.Fatalf("Event = %q, want %q", health.Event, "heartbeat")
	}

	if health.State != "READY" {
		t.Fatalf("State = %q, want %q", health.State, "READY")
	}
	...
	if !health.Hooks["b6"] || !health.Hooks["A"] {
		t.Fatalf("Hooks = %v, expected b6 and A to be true", health.Hooks)
	}

	if got := health.Counters["capture"]; got != 5 {
		t.Fatalf("Counters[capture] = %d, want %d", got, 5)
	}

	if health.TimestampMs != 1711363200000 {
		t.Fatalf("TimestampMs = %d, want %d", health.TimestampMs, 1711363200000)
	}
}
