...
var BASE_INTENT_FILTER_TARGET = {
  className: "com.kakao.talk.activity.f",
  captureMethod: "b6",
  bridgeMethod: "Z5"
};
var CHAT_MEDIA_SENDER_TARGET = {
  className: "bh.c",
  entryMethod: "o",
  processMethod: "t",
  injectMethod: "A"
};
var THREAD_IMAGE_GRAFT_HOOK_SPECS = {
  b6: {
...
var IRIS_IDENTIFIER_PREFIX = "iris:";
...
function sessionIdFromIdentifier(identifier) {
...
function readSessionMeta(snapshot) {
  const extras = snapshot.extras ?? {};
  const sessionId = readStringValue(extras[IRIS_EXTRAS.SESSION_ID]) ?? sessionIdFromIdentifier(snapshot.identifier);
  if (sessionId == null) {
    return null;
  }
  return {
...
function copySessionExtras(source, targetExtras) {
...
var DEFAULT_SWEEP_INTERVAL_MS = 5e3;
function fingerprintUriStrings(uris) {
  if (uris.length === 0) {
    return "empty";
  }
  return `${uris.length}:${uris.join("|")}`;
}
function mergeSessionMeta(previous, next) {
  if (previous == null) {
    return next;
  }
  if (next == null) {
...
var FingerprintSessionStore = class {
  entries = /* @__PURE__ */ new Map();
  ttlMs;
  maxSize;
  sweepIntervalMs;
  lastSweepAt = 0;
  constructor(ttlMs = DEFAULT_TTL_MS, maxSize = DEFAULT_MAX_SIZE, sweepIntervalMs = DEFAULT_SWEEP_INTERVAL_MS) {
    this.ttlMs = ttlMs;
...
// thread-image-graft.ts
var fingerprintStore = new FingerprintSessionStore();
var currentMediaSenderSession = null;
var threadImageGraftHeartbeatTimer = null;
var lastThreadImageEventHealthEmitAtMs = 0;
var ENABLE_HOT_PATH_GRAFT_LOGS = false;
var EMIT_EAGER_EVENT_HEALTH = false;
var THROTTLED_EVENT_HEALTH_INTERVAL_MS = 250;
var hasNativePointer = typeof globalThis.NativePointer !== "undefined";
var JavaBridgeGlobal = hasNativePointer ? (await Promise.resolve().then(() => (init_frida_java_bridge(), frida_java_bridge_exports))).default : null;
var THREAD_IMAGE_GRAFT_HEARTBEAT_INTERVAL_MS = 1e4;
function nowMs(runtime2) {
  if (runtime2 != null && typeof runtime2.now === "function") {
    return runtime2.now();
  }
  return Date.now();
}
function resolveJavaBridge(runtime2) {
  return runtime2.Java ?? JavaBridgeGlobal;
}
...
function buildThreadImageHookStatuses() {
  const hooks = {};
  for (const [hook, spec] of Object.entries(THREAD_IMAGE_GRAFT_HOOK_SPECS)) {
    hooks[hook] = {
      installed: false,
...
function cloneThreadImageHooks(hooks) {
  const copy2 = {};
  for (const hook of Object.keys(hooks)) {
    copy2[hook] = { ...hooks[hook] };
  }
  return copy2;
}
function cloneThreadImageCounters(counters) {
  return {
...
function createThreadImageGraftHealth(timestampMs = Date.now()) {
  return {
    state: "BOOTING",
    ready: false,
...
function setThreadImageGraftState(health, state, timestampMs = Date.now(), ready = state === "READY") {
  health.state = state;
  health.ready = ready;
  health.updatedAtMs = timestampMs;
  return health;
}
function setThreadImageHookMissingReason(health, hook, missingReason) {
...
function touchThreadImageHook(health, hook, timestampMs = Date.now()) {
  health.hooks[hook].lastSeenAtMs = timestampMs;
  health.updatedAtMs = timestampMs;
}
function markThreadImageHookInstalled(health, hook, timestampMs = Date.now()) {
...
function recordThreadImageEvent(health, event, outcome, timestampMs = Date.now(), touchedHooks = []) {
...
function snapshotThreadImageGraftHealth(health, event, timestampMs = Date.now()) {
  return {
    type: "graft-health",
    event,
    state: health.state,
...
function assessThreadImageHookSpec(input, timestampMs = Date.now()) {
...
function emitThreadImageGraftHealth(runtime2, health, event, timestampMs = nowMs(runtime2)) {
  const send = runtime2.send;
  if (send == null) {
    return;
  }
  send(snapshotThreadImageGraftHealth(health, event, timestampMs));
}
function stopThreadImageGraftHeartbeat() {
...
function startThreadImageGraftHeartbeat(runtime2, health) {
  stopThreadImageGraftHeartbeat();
  if (typeof globalThis.setInterval !== "function") {
    return;
  }
  threadImageGraftHeartbeatTimer = setInterval(() => {
    emitThreadImageGraftHealth(runtime2, health, "heartbeat");
  }, THREAD_IMAGE_GRAFT_HEARTBEAT_INTERVAL_MS);
  threadImageGraftHeartbeatTimer.unref?.();
}
function planImageGraft(session, roomId) {
  if (session == null) {
...
function log(runtime2, message) {
  runtime2.send?.(`[graft] ${message}`);
}
function debugLog(runtime2, buildMessage) {
...
function emitThreadImageGraftEventHealth(runtime2, health, event, outcome, timestampMs) {
  const shouldEmit = EMIT_EAGER_EVENT_HEALTH || outcome === "failure" || timestampMs - lastThreadImageEventHealthEmitAtMs >= THROTTLED_EVENT_HEALTH_INTERVAL_MS;
  if (!shouldEmit) {
    return;
  }
  lastThreadImageEventHealthEmitAtMs = timestampMs;
  emitThreadImageGraftHealth(runtime2, health, event, timestampMs);
}
function resolveAvailableMethodTargets(runtime2) {
  const methodsByClass = {};
  const JavaBridge = resolveJavaBridge(runtime2);
  if (JavaBridge == null) {
    return methodsByClass;
  }
  const specsByClass = /* @__PURE__ */ new Map();
  for (const spec of Object.values(THREAD_IMAGE_GRAFT_HOOK_SPECS)) {
    if (!specsByClass.has(spec.className)) {
      specsByClass.set(spec.className, /* @__PURE__ */ new Set());
    }
...
function readSendingLogClassNameFromMethod(method, argumentIndex) {
  try {
...
function resolveSendingLogClassName(runtime2) {
  const JavaBridge = resolveJavaBridge(runtime2);
  if (JavaBridge == null) {
    return null;
  }
...
function resolveAvailableFieldTargets(runtime2, sendingLogClassName) {
  const fieldsByClass = {};
  const JavaBridge = resolveJavaBridge(runtime2);
  ...
  try {
    ...
    const classHandle = sendingLogClass.class;
    const fieldNames = [
...
function resolveThreadImageHookAssessment(runtime2, timestampMs = nowMs(runtime2)) {
  const sendingLogClassName = resolveSendingLogClassName(runtime2);
  return assessThreadImageHookSpec({
    methodsByClass: resolveAvailableMethodTargets(runtime2),
    fieldsByClass: resolveAvailableFieldTargets(runtime2, sendingLogClassName),
    sendingLogClassName
  }, timestampMs);
}
function missingThreadImageRequirements(health) {
  return Object.entries(health.hooks).filter(([, status]) => status.missingReason != null).map(([hook, status]) => `${hook}:${status.missingReason}`);
}
function applyThreadImageHookAssessment(health, assessment, timestampMs = Date.now()) {
  for (const hook of Object.keys(health.hooks)) {
    health.hooks[hook] = {
      ...health.hooks[hook],
...
function finalizeThreadImageGraftInstall(health, timestampMs = Date.now()) {
  const missingRequirements = missingThreadImageRequirements(health);
  if (missingRequirements.length > 0) {
    setThreadImageGraftState(health, "BLOCKED", timestampMs, false);
    return;
  }
  const allHooksInstalled = Object.keys(health.hooks).every((hook) => health.hooks[hook].installed);
  setThreadImageGraftState(health, allHooksInstalled ? "READY" : "DEGRADED", timestampMs, allHooksInstalled);
}
function readIntentIdentifier(intent) {
  if (intent == null) {
    return null;
  }
  try {
...
function readIntentStringExtra(intent, key) {
  if (intent == null) {
    return null;
  }
  try {
...
function readIntentIntExtra(intent, key) {
  if (intent == null) {
    return null;
  }
  try {
...
function readIntentLongExtra(intent, key) {
  if (intent == null) {
    return null;
  }
  try {
...
function applySessionExtras(targetIntent, sourceIntent) {
...
function readUriStrings(values) {
  if (values == null) {
    return [];
  }
  ...
  for (const overload of BaseIntentFilterActivity[BASE_INTENT_FILTER_TARGET.captureMethod].overloads) {
    overload.implementation = function(...args) {
      ...
      if (merged != null) {
        ...
        emitThreadImageGraftEventHealth(runtime2, health, "capture", "success", timestampMs);
        debugLog(runtime2, () => `b6 captured fp=${fingerprint} session=${merged.sessionId} threadId=${merged.threadId ?? "none"} scope=${merged.threadScope} room=${merged.roomId ?? "none"}`);
      } else {
        recordThreadImageEvent(health, "capture", "skip", timestampMs, ["b6"]);
        emitThreadImageGraftEventHealth(runtime2, health, "capture", "skip", timestampMs);
      }
...
function installChatMediaSenderHooks(runtime2, health) {
  const JavaBridge = resolveJavaBridge(runtime2);
  if (JavaBridge == null) {
    return;
  }
  ...
  for (const overload of ChatMediaSender[CHAT_MEDIA_SENDER_TARGET.entryMethod].overloads) {
    overload.implementation = function(...args) {
      ...
      emitThreadImageGraftEventHealth(runtime2, health, "restore", session == null ? "skip" : "success", timestampMs);
      debugLog(runtime2, () => `o entry fp=${fingerprint} session=${session?.sessionId ?? "none"}`);
      try {
        return overload.apply(this, args);
      } finally {
    ...
    overload.implementation = function(...args) {
      ...
      if (currentMediaSenderSession == null) {
        ...
        if (session != null) {
          ...
          emitThreadImageGraftEventHealth(runtime2, health, "restore", "success", timestampMs);
          debugLog(runtime2, () => `t fallback-restore fp=${fingerprint} session=${session.sessionId}`);
        } else {
          recordThreadImageEvent(health, "restore", "skip", timestampMs, ["t"]);
          emitThreadImageGraftEventHealth(runtime2, health, "restore", "skip", timestampMs);
        }
      }
      try {
        return overload.apply(this, args);
      ...
      if (session != null) {
        ...
        if (plan.status === "inject" && plan.injection != null) {
          ...
          try {
            ...
            sendingLog.J1(LongClass.valueOf(StringClass.valueOf(injection.threadId)));
            recordThreadImageEvent(health, "inject", "success", timestampMs, ["A"]);
            emitThreadImageGraftEventHealth(runtime2, health, "inject", "success", timestampMs);
            debugLog(runtime2, () => `A injected session=${session.sessionId} room=${roomId ?? "none"} threadId=${injection.threadId} scope=${injection.threadScope}`);
          } catch (error) {
            recordThreadImageEvent(health, "inject", "failure", timestampMs, ["A"]);
            emitThreadImageGraftEventHealth(runtime2, health, "inject", "failure", timestampMs);
            emitThreadImageGraftHealth(runtime2, health, "readiness", timestampMs);
            log(runtime2, `A injection-failed session=${session.sessionId} error=${String(error)}`);
          }
        } else {
          recordThreadImageEvent(health, "inject", "skip", timestampMs, ["A"]);
          emitThreadImageGraftEventHealth(runtime2, health, "inject", "skip", timestampMs);
          debugLog(runtime2, () => `A skip status=${plan.status} session=${session.sessionId} room=${roomId ?? "none"}`);
        }
      } else {
        recordThreadImageEvent(health, "inject", "skip", timestampMs, ["A"]);
        emitThreadImageGraftEventHealth(runtime2, health, "inject", "skip", timestampMs);
...
function installSendingLogHook(runtime2, health) {
  const JavaBridge = resolveJavaBridge(runtime2);
  if (JavaBridge == null) {
    return;
  }
...
function installThreadImageGraft(runtime2 = globalThis) {
  const JavaBridge = resolveJavaBridge(runtime2);
  ...
  const health = createThreadImageGraftHealth(nowMs(runtime2));
  emitThreadImageGraftHealth(runtime2, health, "health", health.updatedAtMs);
  JavaBridge.perform(() => {
    const hookingAtMs = nowMs(runtime2);
    setThreadImageGraftState(health, "HOOKING", hookingAtMs, false);
    emitThreadImageGraftHealth(runtime2, health, "health", hookingAtMs);
    const assessment = resolveThreadImageHookAssessment(runtime2, hookingAtMs);
    applyThreadImageHookAssessment(health, assessment, hookingAtMs);
    if (assessment.state === "BLOCKED") {
      emitThreadImageGraftHealth(runtime2, health, "readiness", hookingAtMs);
      emitThreadImageGraftHealth(runtime2, health, "heartbeat", hookingAtMs);
      startThreadImageGraftHeartbeat(runtime2, health);
      log(runtime2, `thread-image-graft blocked ${missingThreadImageRequirements(health).join(", ")}`);
      return;
    }
    installBaseIntentFilterHook(runtime2, health);
    installChatMediaSenderHooks(runtime2, health);
    installSendingLogHook(runtime2, health);
    const readyAtMs = nowMs(runtime2);
    finalizeThreadImageGraftInstall(health, readyAtMs);
    emitThreadImageGraftHealth(runtime2, health, "readiness", readyAtMs);
    emitThreadImageGraftHealth(runtime2, health, "heartbeat", readyAtMs);
    startThreadImageGraftHeartbeat(runtime2, health);
    log(runtime2, "thread-image-graft hooks installed (4-hook session-chain)");
  });
  return true;
}
...
export {
  assessThreadImageHookSpec,
  createThreadImageGraftHealth,
  installThreadImageGraft,
  markThreadImageHookInstalled,
  planImageGraft,
  recordThreadImageEvent,
  setThreadImageGraftState,
  snapshotThreadImageGraftHealth
};
